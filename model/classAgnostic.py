...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2):
        ...
        # self.fc = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=1, stride=1, bias=False)
...
def ResNet18(pretrained=False, **kwargs):
    model = ResNetShared(Bottleneck, [2, 2, 2, 2], **kwargs)

    if pretrained:
        model.load_state_dict(model_zoo.load_url('https://download.pytorch.org/models/resnet18-f37072fd.pth'))
    return model
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_planes, final_upsample_mode = 'bilinear', last=1):
        ...
        self.final = nn.Sequential(
            nn.Conv2d(64 + 4*64, last, kernel_size=3, stride=1, padding=1, bias=True),
            nn.Sigmoid(),
...
def decoder(last):
    return Decoder([256, 512, 1024, 2048], last=last)
